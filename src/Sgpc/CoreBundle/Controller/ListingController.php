...
class ListingController extends Controller {
    ...
    public function createAction(Request $request, $id) {
        ...
        if ($form->isValid()) {
            ...
            $em->persist($entity);
            $em->flush();
            if ($parent->getModel() == 'kanban') {
                return $this->redirect($this->generateUrl('sgpc_project_kanban', array('id' => $entity->getProject()->getId())));
            } else {
                return $this->redirect($this->generateUrl('sgpc_project_scrum', array('id' => $entity->getProject()->getId())));
            }
        }
